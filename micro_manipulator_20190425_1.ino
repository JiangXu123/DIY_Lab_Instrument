...
int Speed1 = 3;    //Speed1 is the L239D on time
int Speed2 = 3;   //Speed2 is the L239D off time
void timerIsr() {
  encoder->service();
}
...
void Joystick_Click() {
  CLICK_COUNT++;
  if(CLICK_COUNT == 5){ //each click will toggle CLICK_COUNT between 1 and 4
    CLICK_COUNT = 1;}
}
...
void loop() {
  ...
  if(digitalRead(right_turn) == LOW){   // these codes are used to control the L293D to control the turn of the geared DC motor, which is used to control the turn of the capiliary.
    Serial.println("right turn clicked");
    digitalWrite(L293D_Enable1, HIGH);
    digitalWrite(L293D_Input1, HIGH);
    digitalWrite(L293D_Input2, LOW);
       // the ratio between Speed1 and Speed2 controls the rotation speed of the DC motro
    }
      
  if(digitalRead(left_turn) == LOW){
    Serial.println("left turn clicked");
    digitalWrite(L293D_Enable1, HIGH);
    digitalWrite(L293D_Input1,LOW);
    digitalWrite(L293D_Input2,HIGH);                                                                                                             
    }
  if(digitalRead(left_turn) == HIGH && digitalRead(right_turn) == HIGH){
    digitalWrite(L293D_Enable1, LOW);
    digitalWrite(L293D_Input1, LOW);
    digitalWrite(L293D_Input2, LOW);
    }
    
  if(digitalRead(pump_asp) == LOW){
    pump_Asp();
    }
  if(digitalRead(pump_blow) == LOW){
    pump_Blow();
    }
}
